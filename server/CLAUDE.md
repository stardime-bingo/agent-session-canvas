...
成员清单
index.mjs: HTTP 总入口(:4517 仅本机)，路由表驱动 API + SSE 广播(图签名 diff：会话增删/状态翻转/改名/接力产出才举旗，
...
ai.mjs: 认知层。extractDigest 三流事件抽取(对白+工具轨迹+报错现场)，轻档(命名/摘要)读首尾、深档(接力)加中段切片共 ~900KB 窗口；
