...
index.mjs: HTTP 总入口(:4517 仅本机，AGENT_CANVAS_PORT 仅供并行测试实例)，路由表驱动 API（/api/session 同时返回开场 digest 与独立尾部 endingDigest；
  ...
  手绘层(edge/note/board/drawing → data/canvas.json；图片 → drawing-files.json；node-from-edge 原子建对象+连线)/ws-rename(工作区别名)/rename(看板层+按原生格式写回本体:
...
canvas-repository.mjs: 画布场景事务仓；在全局 scene lock 内统一 strict read、sceneToken CAS、board/district carry journal、drawing receipt/status 与崩溃恢复，拒绝旧快照覆盖
drawing-files.mjs: Excalidraw BinaryFiles 图片资产仓；校验 data URL 与文件 ID，独立原子落 drawing-files.json，避免普通笔画重写大图
canvas-actions.mjs: 落空连线的纯动作内核；便签/画板与手动边构造为同一 canvas 快照，同写同成败
...
依赖方向: index → scanner/launcher/ai/backfill/canvas-repository/drawing-files/canvas-actions → llm/adapters/store，单向无环。
