...
index.mjs: HTTP 总入口(:4517 仅本机，AGENT_CANVAS_PORT 仅供并行测试实例)，路由表驱动 API（/api/session 同时返回开场 digest 与独立尾部 endingDigest；
  ...
  场景快照(POST /api/scene 全量 LWW 落盘+SSE 回播 writerId；POST /api/drawing-files 资产先行)/reveal(尊重 macOS 默认文件管理器)/ws-rename(工作区别名)/rename(看板层+按原生格式写回本体:
...
file-viewer.mjs: macOS 文件管理器边界——读取全局 NSFileViewer；第三方默认用 bundle id 精确打开，Finder/未配置走系统 open，失效偏好自动回退
