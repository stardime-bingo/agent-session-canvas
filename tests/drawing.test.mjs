...
test('4518 新合同夹具：不派发合成输入、timer 轮询后台可跑、七链具名', () => {
  ...
  for (const name of ['coldFrame', 'rapidMutations', 'openSelection', 'exitRestores', 'editorMounts', 'emptyExitFast', 'globalUndo', 'backgroundFlush']) {
    assert.match(fixture, new RegExp(`checks\\.${name}`), `新合同七链必须具名：${name}`);
  }
});
...
test('4518 帧探针只能驱动真实 open/exit，故障只能从 InkWorld exporter 注入', () => {
  ...
  assert.match(fixture, /probe\(\)\.openDrawing\(/,
    'opening 必须调用 production openDrawing');
  assert.match(fixture, /probe\(\)\.exitDrawing\(/,
