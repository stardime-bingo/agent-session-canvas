...
- layout-quality：production `FlowCanvas + SceneStore + TopBar` 的匿名乱序地图；真实点击“智能整理”，直接读取 React Flow DOM 验证工作区活跃度行对齐、1–4 条平衡车道、画板压缩并参与排布、街区终点持久化、容器墨迹随行、便签不动及一键撤销；随后让已整理街区增长，确认后继顺延且仍零碰撞。首帧绑定目标节点 transform 提交，并采集同步耗时与 Long Task。
