...
- “选绘图”后可直接点击矩形、椭圆、菱形的内部或边缘；线、箭头与手绘线仍按实际笔迹命中。普通模式继续允许空心图形内部穿透到底下的卡片。
- 本地绘图草稿与当前画布冲突时不会覆盖或自动删除；提示条可查看无正文元数据、下载本地 JSON，或经二次确认后精确放弃旧草稿。
