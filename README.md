...
![AGENT 会话指挥塔匿名产品画布：会话卡、画板、便签与自研墨迹](./docs/assets/agent-session-canvas-hero.png)

> 上图由隔离在 4518 的 production `FlowCanvas` 匿名场景生成，不含真实会话、本机路径或用户资产。
...
- 生成摘要与接力提示词，并从原会话续开；带接力新开时明确选择 Claude Code 或 Codex，不再隐式沿用源工具。
- 过滤子智能体、空壳与 headless 自噪，automation 多次运行会自动聚合。
- 用零绘图库依赖的自研 SVG 墨迹直接在同一画布标注；卡片、便签、画板与墨迹共用相机和撤销链。
...
- `V` 选择，`P` 画笔，`R` 矩形，`O` 椭圆，`A` 箭头，`T` 文字，`E` 橡皮，`Esc` 收起工具；一笔橡皮只占一步撤销。
- 选择态可框选、`Shift` 多选、批量移动/缩放/旋转/删除/改样式；`⌘C/⌘V` 复制粘贴，`Alt` 拖动复制。
- 双击文字就地编辑；图片文件可直接粘贴或拖进画布，先即时出现占位，再在后台内容寻址入仓。
- 普通模式的空心图形内部继续穿透到底下卡片；大块实心底板自动沉层，不制造隐形玻璃。
- 右上角同步点显示已保存/未同步/同步中/自动重试；daemon 短暂离线不阻塞任何输入。
...
系统已安装 Swift 工具链时，安装脚本会在 `~/Applications/会话指挥塔.app` 放置一个原生 Mac 控制器。把它固定到 Dock 后，单击即可在服务停止时自动启动并打开看板；菜单栏图标会显示运行状态，并提供重新启动、停止和诊断。缺少 Swift 时会保留下面的 Finder/命令行入口并给出提示，不影响主服务安装。

控制器设置里可以分别关闭 Dock 图标和菜单栏图标。如果两者都隐藏，服务仍会继续运行；从“应用程序”再次打开控制器，或双击下面的恢复入口，就能重新显示控制项。退出控制器也不会停止后台服务。

仓库内同时保留 Finder 可双击的薄入口；它们都只调用同一份 launchd 控制脚本，不复制服务逻辑：

- `scripts/打开会话指挥塔.command`
- `scripts/启动会话指挥塔.command`
- `scripts/停止会话指挥塔.command`
- `scripts/显示会话指挥塔控制器.command`
...
- `drawing-files.json`：粘贴或拖入画布的内容寻址图片资产
- `layout.json`：手工布局
...
画布只有一份场景文档。鼠标动作同步写入内存中的 SceneStore，React 直接把文档渲染成 SVG；网络与磁盘只在 300ms 防抖后后台冲刷，失败会无限退避重试。没有导出帧、双相机或“保存后才显示”的等待。

自研墨迹只依赖 React 与浏览器 SVG：笔迹、形状、箭头、文字、图片、命中、框选、缩放和旋转都在本仓库的纯函数内核中。旧磁盘格式继续兼容，升级无需迁移真实画布数据。

## 架构

- `server/`：零依赖 Node daemon、双适配器扫描、AI 路由与本地 HTTP/SSE
- `web/`：React 18 + React Flow + 零额外绘图库的自研墨迹层
- `shared/`：容器承载等前后端共用纯几何原语
