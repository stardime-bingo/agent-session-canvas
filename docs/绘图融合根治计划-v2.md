...
> 归档：本计划已完成并冻结为工程决策与验收记录；保留原路径以维持 `.loop/backlog` 的证据引用，不再作为进行中任务清单。
> 状态：LE-011R、LE-011B、LE-012、LE-013、LE-014 均已独立验收并合入；Loop 最终全局门与 4518 Computer Use 视觉烟测已通过
...
> 已接受实现：LE-011R 候选 `79a780725c45745853554842de18df650c2ffe94`，合并提交 `15063eeb3b0ca904684cad247b75217dcf480faf`
> 当前边界：交互验收只用合成 4518；4517 仅最终只读打开与 console 检查，禁止坐标操作；不触碰真实数据和 launchd
...
2. synthetic index 不含 inline script/style；真实 production index 只允许逐字校验的 `window.EXCALIDRAW_ASSET_PATH = '/';` bootstrap；
3. 只读静态 server 只暴露 index/assets；
4. `/api/*` 一律 403；
5. 不存在 HMR、React refresh、`/@fs`；
6. synthetic CSP 保持 `script-src 'self'`；production CSP 只额外加上述 bootstrap 的精确 SHA-256，不得含 `unsafe-inline` 或普通 `unsafe-eval`；
...
LE-014 在此之上加一个不替代 carry 四场景的全局收敛门：同一个只读 4518 静态服务同时承载真实 `web` production entry 与现有 interaction build。第一个 fresh context 只在应用执行前注入最小 graph fetch 和 `/api/events` EventSource，验证真实 `.canvas-root`、严格 CSP 与零原生诊断；第二个 fresh context 独立验证 `concurrent/revision/opening/closing/coldError/warmError/lateIsolation` 七链及 DOM 镜像。两个 build 合并时同名资产必须字节一致，两个真实 subset worker entry 分别先通过识别，再进入同一 worker-only CSP allowlist。
...
LE-014 中 Computer Use 截图、4517 只读 console 与最终 manifest 继续由 candidate 外部 observer/Judge 持有；candidate 本身不生成、不存放、不自验这些证据。自动闸门只访问 4518 synthetic/注入数据，不访问 4517、真实数据、launchd 或远端。
...
- 不引入 SceneStore 或数据迁移；drawing commit 复用既有跨进程 scene lock、strict read、CAS 与 durable journal/receipt 原语。
- 客户端只上传相对 `previousSuccessful` 的 BinaryFiles delta；服务端先持久化新资产，再以无 base64 的 prepared/committed journal 提交完整 drawing 引用。同 fileId 只接受同一二进制内容并保留首份 canonical metadata。
- `opId + commandHash` 保证幂等；响应丢失后查询 status。只有 commit 与 status 同时不确定才进入 `AUTHORITY_UNKNOWN`，明确 unknown 保持可重试。
- graph 在同一 scene lock 内恢复 journal、校验引用并返回 canvas/files；孤儿裁剪与 receipt 后 journal 清理均为 best-effort，不得降级已 durable 的提交。
- 临时编辑草稿只写浏览器 IndexedDB 的单 active journal：`requestId + epoch + seq` 条件身份、`sceneToken + closureFingerprint + baselineFingerprint` 精确匹配才作为局部 editSeed 恢复；若当前 baseline 已等于 `mergedFingerprint`，只判为旧草稿已提交并清理。首次 Excalidraw 水合前不上报；pagehide/visibility hidden 只 flush 本地。
- durable commit 后等待完整 merged SVG 进入 DOM 才按 request/epoch 条件清理；若 closing 失败，编辑器继续以 advanced transaction 和提交回执的新 sceneToken 原子覆盖续写 journal，不先删除旧记录制造崩溃空窗。

### LE-013 实现落点

- 自动沉底撤销不再恢复整份 pre-commit 快照；动作到达提交队首时只在票据成功代仍匹配的前提下，把 `sunkIds` 及其 `containerId` 绑定文字设回浮层。底板本身、后续新元素与 BinaryFiles 全部保留；普通后续成功令旧票据失效，后续失败不换代。
- closing 的静态帧交接失败可在同一 advanced transaction 上重试，并把票据推进到本次成功快照；事务身份或队首基线不匹配时不得接管别代票据，已删除或手动浮起的 ID 从票据裁掉。
- `UIHost` 的 toast 栈是 `role=status`、`aria-live=polite`；动作按钮使用原生 button，单次认领防双触发。普通提示保留 3.4s，动作提示保留 30s，hover/focus 暂停并在离开后只恢复剩余时长。
- 4518 只生产构建 allowlist 中的 synthetic canvas/carry fixture 到临时 dist，静态服务只开放 `index.html` 与 `assets/`，并拒绝写方法、`/api`、`/data`、`/@fs`、`/.git`、编码 traversal 与越界 realpath；始终只绑定 `127.0.0.1:4518`。
- 300/800 性能计时在 InkWorld DOM-ready 后再等双 rAF，以实际 paint 边界结束；红线以该真实边界校准，不用放宽掩盖回退。interaction 内存提交使用 `{ previousSuccessful, next }` 并递增 synthetic `sceneToken`，不发 API、不落持久化。
- build gate 分别从 app main、Excal prod 与 subset worker entry 递归遍历全部静态依赖；任一缺失依赖失败，worker core 不得进入 main/prod。合法小型共享 bridge 受显式单块与总量预算约束。

### LE-014 实现落点

- `--fixture=prod` 先独立 build 真实 `web` 与 synthetic interaction，再以字节级 collision gate 合并 assets；两个 build 各自必须恰好识别一个真实 subset worker entry。
- production 首页仅用精确 bootstrap SHA-256 放行唯一 inline script，`connect-src 'none'`；最小 graph/EventSource 在 fresh browser context 的 init script 内注入，不产生真实 API 网络。
- production boot 与 interaction 七链使用两个 fresh context；二者都必须零 console warning/error、page error、requestfailed、外联和 API resource。closing 的最终零导出帧只能由同一 closing 的较早成功导出签名精确证明复用。
- `npm run verify:global` 非递归地先执行 production build，再以单并发度运行全部 Node/browser tests；它不调用自身，也不通过 `npm test` 再嵌套一层。
- Computer Use、4517 只读 console、candidate manifest 及最终 Judge 证据属于 candidate-external 层；本实现不触碰 4517、真实数据、launchd 或远端。

Loop Engine 的 backlog ID 只接受 `LE-数字`，因此新一轮 run 内继续使用 `LE-011` 作为
`LE-011B` 的机器 ID；summary、module 和验收合同必须明确写出 `LE-011B`。旧 run
`loop-20260715T084215Z-ded88141` 只封存 LE-011R 的历史证据，不作为后续 backlog 真相。
