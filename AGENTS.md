...
- **绘图双平面（真融合）**: 元素右键可"沉到卡片下面"(customData.below)——沉层经 exportToCanvas 静态垫在点阵与卡片之下、
  命中与点击全让位卡片，区域底板从此是背景不是玻璃；浮层批注照旧在上；编辑激活时全量合流进活实例、退出再分流；
  选绘图点空白（未命中任何笔迹）自动放行回看板并把点击转交底下对象——绘图模式不再是监狱
