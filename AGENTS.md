...
tests/ - node:test 回归；fixtures/canvas-acceptance 覆盖 4518 挂载/拖动/交互，scene-sync-acceptance 用临时 daemon 验双标签与 pagehide；archive/ 保存 v17 历史合同
...
- 自动整理只动几何并保留人工归属；画板/街区缩放要补偿成员坐标；布局不得把锚点误当重叠许可
