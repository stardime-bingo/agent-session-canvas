...
拖动语义分层：会话卡固定 → 工作区在街区容器内自由整理(碰一个快照全街区,拖动中目标容器高亮，只改看板归属不动本地文件) → 街区整块搬家。
...
⑤ 不可逆动作必过确认——整理(清 layout)/删会话(活跃门禁二次确认)/删画板/删有字便签。

成员清单
index.html: 入口壳
src/main.jsx: 挂载点，只挂载不做逻辑
src/App.jsx: 总装线，全局状态 + 过滤管道(工具/状态/时间裁剪到会话粒度/搜索) + SSE 举旗 + 画板/便签动作分发 +
  空态指路牌(筛空给"清除筛选"出口) + 整理确认流 + 绘图时动作岛让位
src/api.js: 数据访问唯一通道，组件不直接碰 fetch
src/util.js: 展示工具箱，relTime/shortPath/fmtSize + TOOL_META/STATUS_META 文案常量
src/ui.jsx: UI 原子库——Icon 单色 SVG 图标集(全站去 emoji)、toast()/confirmPop() 单例 + <UIHost/> 宿主、
...
src/canvas/FlowCanvas.jsx: 画布引擎，统一容器模型（自动街区+用户画板同构）：归属律 layout.d 手动指定>路径推断，
  ...
  connecting 类手动挂载(xyflow 无原生根类)；纯展示图例点击穿透不遮挡节点入口
src/canvas/layout.js: FlowCanvas 的纯布局内核，不读写磁盘；手工锚点优先的工作区打包、增量成员补位、容器增长避让，
  保证自动整理与地形增长后的工作区/街区无重叠，可由 node:test 用合成数据直接证伪
...
src/canvas/BoardNode.jsx: 用户自建画板（一等容器），导出 BOARD_COLORS 五色表，可选中可连线，
  InlineEdit 改名、色盘换色、删除走确认流
src/canvas/DistrictNode.jsx: 街区真容器，与画板同皮肤(container-face + CSS 变量注色)，可选中可连线，
...
src/canvas/SessionNode.jsx: 会话卡片节点(memo)，工具色脊柱 + 人话标题两行(InlineEdit 双击改名) + 状态灯/自动化角标，
  hover 提边选中蓝锚
src/panels/TopBar.jsx: 右上动作岛：有新活动举旗(服务端签名 diff,纯写盘不举)/整理(过确认)/批量命名(进度轮询)/重扫
