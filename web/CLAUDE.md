...
src/canvas/drawing.js: 绘图元素/图片文件的纯快照内核 + 目标容器/绑定/箭头端点/画框/分组递归闭包、selection/new 局部事务、持久化后 ownership advance、visible committed hole 与全量 merge + 编辑器水合就绪握手纯状态步 + canvasGeometryAllowed/opening preparation + committed 过滤/删除/沉浮/平移不可变变换 + createDrawingCommitQueue 串行提交（到队首才变换、失败不毒后续、
