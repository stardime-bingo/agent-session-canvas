...
src/canvas/InkTools.jsx: 墨迹交互层 useInkTools——拖画/文字击键/橡皮笔迹 coalesce mutate，文字 pointerdown 阻止默认焦点回抢；框选/Shift 多选/批量移动缩放旋转删除改样式、ClipboardEvent 复制粘贴、Alt 拖、图片粘贴/拖入、V/P/R/O/A/T/E；armed 滚轮直改 RF 相机、空格让路原生平移
...
src/panels/DetailPanel.jsx: 右侧详情岛（加载失败自动退避重连）——标题(就地改名)→一键续开→HANDOFF 接力
  (内置轻档生成 + 交接三件套 skill 终端出口 + Claude/Codex 显式双接班入口)→DETAIL 元信息→删除→CONTEXT 摘要+digest→STOP 停止点→RUNS 实例
