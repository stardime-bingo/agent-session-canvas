...
视口手势不再驱动 Excalidraw 或绘图重导出；双表征即残影之源。
...
src/App.jsx: 总装线，全局状态 + 过滤管道(工具/状态/时间裁剪到会话粒度/搜索) + SSE 举旗 + 画板/便签/落空连线动作分发 +
  画布终端框(ContextFrame)开合 + drawingCommit/drawingPersisted 原子回写 elements+files + 空态指路牌(筛空给"清除筛选"出口) +
...
src/canvas/FlowCanvas.jsx: 画布引擎，统一容器模型（自动街区+用户画板同构）：归属律 layout.d 手动指定>路径推断，
  ...
  进绘图先清 RF 选中防 Delete 双雷并立即封住普通提交与节点几何（drawing-opening overlay + nodesDraggable=false），排空串行队列后以最后成功的 elements/files 同快照作 editSeed；
  geometryPending 的整理事务在开编辑前优先完成，opening 时整理/撤销即时让位，active 时全局整理仍可点但先 await flush+exit；编辑期间拒绝普通提交，退出 flush 快照同步回队列后才解门卸载；
  空画布点选绘图给指路 toast；普通态命中直接读 committed 数组，删除/沉浮/承载通过稳定串行队列基于上一成功快照提交，
  exitToCanvas 等落盘后放行转交点击）；
...
src/canvas/drawing.js: 绘图元素/图片文件的纯快照内核 + 编辑器 API/水合就绪握手纯状态步 + canvasGeometryAllowed(opening/drawing/pending 三锁) 与 canvasGeometryPreparation(blocked/exit-drawing/ready) + committed 过滤/删除/沉浮/平移不可变变换 + createDrawingCommitQueue 串行提交（到队首才变换、失败不毒后续、
...
src/canvas/InkWorldLayer.jsx: 普通态 committed ink compositor——按 below 分流并行导出两张 SVG，旧快照保留到两张新图都就绪后同次 React commit 替换；
  通过 ViewportPortal 继承 React Flow 唯一相机，只因 elements/files 变化导出，所有快照 pointer-events:none
...
src/canvas/DrawLayer.jsx: 仅编辑态挂载的 Excalidraw 工作副本(React.lazy 拆包)；普通态不再常驻实例。单握手 ref 等待 API 与首次 onChange 水合，只回传一次稳定 controller，
  首次水合的默认 selection 不回灌，只有 ready 前已就绪的后续 change 才上报工具；
  ref/onReady 共用该对象；坐标契约 scroll=viewport/zoom，保留原生选择与描边/背景/透明度/图层属性岛；controller 暴露视口/工具/选中/快照与可等待 flush，
  初始 elements/files 只接收 FlowCanvas 排空队列所得 editSeed；flush 落盘并回写 elements+files 后才允许卸载；
  失败时保留编辑现场和旧 committed 快照，pagehide 仍尽力保存副本；选绘图空点经 onExitToCanvas 走同一 flush 门后放行
