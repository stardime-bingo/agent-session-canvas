...
增量布局铁律：手工位置是优先锚点但不是重叠许可；后方成员/容器必须确定性顺延避让，
`layout-reconcile.js` 以同一只读投影携带容器墨迹，页面加载不写；下一次几何/绘图输入前 history:false 原子追平。
...
src/canvas/FlowCanvas.jsx: 画布引擎总装（≤800 行）——统一容器模型、归属律 layout.d>路径推断、容器弹性生长、
  ...
  世界装配 doc.drawing/drawingFiles 交 InkLayer、顶栏 N/B/V/P/R/O/A/T/E/F 可见键帽、4518 只读动作探针 seam；352 节点夹具以状态点/22 条关系线/画板/墨迹挂真实 FlowCanvas，由浏览器同时取空闲零 Paint、held-pointer 与松手零跳变 trace，并兼容 Chrome 任务事件名，匿名 hero 也只做 production FlowCanvas 的数据与构图层
...
src/canvas/InkLayer.jsx: 墨迹渲染层——元素直出 SVG（freedraw 的相对 points 必须叠加元素 x/y；沉/浮两平面 + 多选框/八手柄/旋转柄），ViewportPortal 共用唯一 RF 相机，每元素带 data-ink-element-id 供承载桥；没有导出没有帧
src/canvas/InkTools.jsx: 墨迹交互层 useInkTools——拖画/文字击键/橡皮笔迹 coalesce mutate，创作工具点已有墨迹直接转选择并沿用同次拖动、文字 pointerdown 阻止默认焦点回抢；框选/Shift 多选/批量移动缩放旋转删除改样式、ClipboardEvent 复制粘贴、Alt 拖、图片粘贴/拖入、V/P/R/O/A/T/E；armed 滚轮直改 RF 相机、空格让路原生平移
...
src/canvas/layout-reconcile.js: 增长避碰投影桥——渲染时同投影携带容器墨迹，下一次几何/绘图动作前 history:false 原子提交
src/canvas/menus.jsx: 七套右键菜单 + 三个删除流程 + 交接三件套入口——目录统一用系统默认文件管理器并有回执；菜单、节点按钮、详情面板共用同一条河
...
src/panels/DetailPanel.jsx: 右侧详情岛（加载失败自动退避重连）——标题(就地改名)→一键续开→HANDOFF 接力
  (内置轻档生成 + 按钮内 AI 忙碌状态点 + 交接三件套 skill 终端出口 + Claude/Codex 显式双接班入口)→DETAIL 元信息(醒目默认文件管理器按钮)→删除→CONTEXT 摘要+digest→STOP 停止点→RUNS 实例
