...
src/canvas/FlowCanvas.jsx: 画布引擎总装（≤800 行）——统一容器模型、归属律 layout.d>路径推断、容器弹性生长、
  ...
  世界装配 doc.drawing/drawingFiles 交 InkLayer、顶栏 N/B/V/P/R/O/A/T/E/F 可见键帽、4518 只读动作探针 seam；352 节点夹具以状态点/22 条关系线/画板/墨迹挂真实 FlowCanvas，由浏览器同时取空闲零 Paint、held-pointer 与松手零跳变 trace，并兼容 Chrome 主线程任务事件名，匿名 hero 也只做 production FlowCanvas 的数据与构图层
