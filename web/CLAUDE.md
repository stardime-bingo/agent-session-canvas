...
**v17 根治进行中**：LE-008/009 已收口 rendered frame 与稳定局部槽；LE-010 以可渲染同层输入盾、FlowCanvas 单一快捷键 owner 和 z:7 缩放岛收口相机尾窗，Shift+1/2/3 在单 RF 相机中统一折叠为全景 fit。容器墨迹松手追赶、草稿/资产持久化等后续项仍未达标；在 `docs/绘图融合根治计划-v2.md` 的 LE-008～LE-014 全部通过前不得宣告根治。
...
src/api.js: 数据访问唯一通道，组件不直接碰 fetch；layoutBatch 支持普通几何合并，containerBatchCarry 专管整理/撤销的布局+锚定墨迹原子事务，绘图图片/落空连线/上下文倒序分页(contextPage)走专线
